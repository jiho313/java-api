...
public class BankingApplication {
	...
	private void 전계좌조회() {
		...
		System.out.println("### 예금주명으로 입력해서 전체 계좌정보를 조회하세요.");
		
		// 계좌정보를 조회할 예금주명을 입력받는다.
		System.out.print("### 예금주: ");
		String name = reader.readString();
		
		// BankingServicd객체의 getMyAllAccounts(String name) 메소드로 전달해서 호출한다.
		// 예금주명으로 조회된 계좌정보들을 List<Account> 객체로 반환받는다.
		List<Account> accounts = service.getMyAllAccounts(name);
		
		if (!accounts.isEmpty()){ // 안에가 비었으면 true를 반환하는 isEmpty메소드
	...
	private void 계좌상세조회() {
		...
		System.out.println("### 계좌정보, 비밀번호를 입력하세요.");
